...
|OBJECT|执行对象的`hashCode()`，注意，曾经有人误认为是对象在JVM中的物理内存地址，但很遗憾他不是。但他能帮助你简单的标记当前执行方法的类实体|
|CLASS|执行的类名|
|METHOD|执行的方法名|
...
- 条件表达式
	...
	* 我只需要观察特定参数，但是 tt 却全部都给我记录了下来
    ...
    条件表达式也是用 `OGNL` 来编写，核心的判断对象依然是 [`Advice`][https://alibaba.github.io/arthas/advice-class.html] 对象。除了 `tt` 命令之外，`watch`、`trace`、`stack` 命令也都支持条件表达式。
