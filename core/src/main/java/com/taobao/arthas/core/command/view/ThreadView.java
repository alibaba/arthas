...
public class ThreadView extends ResultView<ThreadModel> {
    ...
    public void draw(CommandProcess process, ThreadModel result) {
        if (result.getThreadInfo() != null) {
            ...
            ThreadInfoExt[] threadInfos = result.getBusyThreads();
            for (ThreadInfoExt info : threadInfos) {
            ...
            String content = ThreadUtil.drawThreadInfo(threadStats, process.width(), height);
